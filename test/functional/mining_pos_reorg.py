...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected CROM and zCROM supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zCROMsupply']:
                assert_equal(node['zCROMsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check CROM and zCROM supply at the beginning
        # ------------------------------------------
        # zCROM supply: 2 coins for each denomination
        ...
        # CROM supply: block rewards minus burned fees for minting
        ...
        # Verify that CROM and zCROM supplies were properly updated after the spends and reorgs
        self.log.info("Check CROM and zCROM supply...")
