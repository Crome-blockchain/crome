...
{
    // double check that there are no double spent zCROM spends in this block or tx
    if (tx.HasZerocoinSpendInputs()) {
        ...
        //This zCROM serial has already been included in the block, do not add this tx.
        if (fDoubleSerial) {
            return false;
        }
    }
...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zCROM spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // zCROM check to not include duplicated serials in the same block.
