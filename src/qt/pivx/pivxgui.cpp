...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Crome client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Crome"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Crome"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
