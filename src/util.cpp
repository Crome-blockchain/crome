...
const char * const PIVX_CONF_FILENAME = "crome.conf";
const char * const PIVX_PID_FILENAME = "crome.pid";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\crome
// Windows >= Vista: C:\Users\Username\AppData\Roaming\crome
// Mac: ~/Library/Application Support/crome
// Unix: ~/.crome
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Crome";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\cromeParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\cromeParams
    // Mac: ~/Library/Application Support/cromeParams
    // Unix: ~/.crome-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CromeParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "CromeParams";
#else
    // Unix
    return pathRet / ".crome-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty crome.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override crome.conf
